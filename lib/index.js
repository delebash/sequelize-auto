  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(!self.options.typescript){
        ...
        text[table] += "const sequelizeClient = app.get('sequelizeClient');\n";
        text[table] += spaces + "const" + tableName + " = sequelizeClient.define('" + tableName + "', {\n";
      } else {
      ...
      text[table] += spaces + spaces  + "  beforeCount(options) {\n";
      text[table] += spaces + spaces  + "   options.raw = true;\n";
      text[table] += spaces + spaces  + "  ),\n";
      ...
      text[table] += "// eslint-disable-next-line no-unused-vars\n";
      text[table] +=  tableName + ".associate = function (models) {\n";
      text[table] += "// Define associations here\n";
      text[table] +=  "// See http://docs.sequelizejs.com/en/latest/docs/associations/\n";
      text[table] += spaces + ");\n\n";
      text[table] +=  "return " + tableName + ";\n";
      text[table] += "};";
